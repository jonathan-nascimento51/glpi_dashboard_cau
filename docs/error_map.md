...
| `ModuleNotFoundError` ao rodar scripts | Dependencias ausentes ou ambiente virtual desativado | Ative o venv, rode `pip install -r requirements.txt -r requirements-dev.txt` e opcionalmente `-r requirements-full-tests.txt` então `pip install -e .` (pacotes estão em `src/`) | "Crie ou ative o venv e reinstale dependencias" |
