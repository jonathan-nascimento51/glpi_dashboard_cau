    ...
    async def fetch_tickets(
        ...
        async with self.glpi_session as session:
            while has_more_data:
                ...
                logger.info(
                    f"Fetching tickets from GLPI: range={params['range']}, since={since_date}"
                )
                try:
                    ...
                    logger.info(
                        f"Fetched {len(tickets_page)} tickets. Total fetched: {len(all_tickets)}"
                    )
                    ...
                    logger.info(
                        "GLPI search endpoint returned 404, likely no more data or invalid query."
                    )
                    has_more_data = False
                except GLPIAPIError as e:
                    logger.error(
                        f"Error fetching tickets from GLPI API: {e.status_code} - {e.message}"
                    )
                    has_more_data = False  # Stop on API errors
                except Exception as e:
                    logger.error(
                        f"An unexpected error occurred during GLPI ticket fetch: {e}"
                    )
