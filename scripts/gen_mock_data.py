...
def generate_tickets(count: int, null_rate: float = 0.0, error_rate: float = 0.0) -> List[Dict[str, Any]]:
    """Return a list of fake GLPI tickets."""
    return [_generate_ticket(i + 1, null_rate, error_rate) for i in range(count)]


def paginate(tickets: List[Dict[str, Any]], page_size: int = 50) -> List[Dict[str, Any]]:
    """Return paginated structure for tickets."""
    pages = []
    for start in range(0, len(tickets), page_size):
        page = start // page_size + 1
        pages.append({"page": page, "tickets": tickets[start : start + page_size]})
    return pages
...
def main() -> None:
    ...
    parser.add_argument("--null-rate", type=float, default=0.0, help="Probability for null fields")
    parser.add_argument("--error-rate", type=float, default=0.0, help="Probability for missing fields")
    parser.add_argument("--output", type=Path, default=Path("mock/sample_data.json"), help="Destination JSON file")
    parser.add_argument("--page-size", type=int, default=50, help="Page size for pagination metadata")
    args = parser.parse_args()

    tickets = generate_tickets(args.count, args.null_rate, args.error_rate)
    data = {"data": tickets, "pages": paginate(tickets, args.page_size)}
